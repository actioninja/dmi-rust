...
impl RawGenericChunk {
	pub fn load<R: Read>(reader: &mut R) -> Result<RawGenericChunk, error::DmiError> {
		...
		if u32::from_be_bytes(crc) != recalculated_crc {
			...
			let crc_values = u32::from_be_bytes(crc);
			return Err(error::DmiError::Generic(format!("Failed to load Chunk of type {chunk_name}. Supplied CRC invalid: {crc:#?}. Its value ({crc_values}) does not match the recalculated one ({recalculated_crc}).")));
		}
